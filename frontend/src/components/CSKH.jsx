...
function ChatbotAI() {
  ...
  const callGeminiAPI = async (userInput, retries = 3) => {
    try {
      ...
      console.log("Sending Gemini API request with input:", userInput);
    ...
    try {
      ...
      if (userInput.includes("hủy vé") && userInput.includes("có")) {
        ...
        if (ticketMatch) {
          ...
          return `Vé ${ticketId} đã được hủy thành công! Bạn cần hỗ trợ gì thêm không?`;
        }
      ...
      if (
        ...
        const ticketData = {
          route: { start_location: "Hà Nội", end_location: "Sài Gòn" },
          departure_date: userInput.match(/\d{2}\/\d{2}/)[0],
        };
        const newTicket = await ticketService.createTicket(ticketData);
        return `Vé của bạn đã được đặt thành công! Mã vé: ${newTicket.id}. Bạn cần in vé hay kiểm tra thêm thông tin?`;
      }
      ...
      if (userInput.includes("vé xe") || userInput.includes("đặt vé")) {
        ...
        if (!locations || locations.length === 0) {
          return "Hiện tại chưa có danh sách địa điểm. Vui lòng thử lại sau hoặc liên hệ hotline 1900 1234.";
        }
        return `Chào bạn! Bạn muốn đặt vé xe đúng không? Hiện tại mình có các điểm đi/đến: ${locations
          .map((loc) => loc.name)
          .join(", ")}. Hãy cho mình biết điểm đi, điểm đến và ngày khởi hành để mình kiểm tra nhé! Ví dụ: Hà Nội - Sài Gòn, ngày 01/05.`;
      }

      // Check schedule
      if (userInput.includes("lịch trình") || userInput.includes("chuyến xe")) {
        ...
        return `Bạn muốn xem lịch trình các chuyến xe phải không? Mình có các tuyến: ${routes
          .map((route) => `${route.start_location} - ${route.end_location}`)
          .join(", ")}. Hãy cho mình biết tuyến bạn quan tâm (ví dụ: Hà Nội - Đà Nẵng), mình sẽ kiểm tra ngay!`;
      }
      ...
      if (userInput.includes("giá vé")) {
        const routes = await routeService.getRoutes();
        if (!routes || routes.length === 0) {
          return "Hiện tại chưa có tuyến đường nào để kiểm tra giá vé. Bạn có thể thử lại sau nhé!";
        }
        return `Mình rất vui được hỗ trợ! Giá vé sẽ tùy thuộc vào tuyến đường và loại xe bạn chọn. Hiện mình có các tuyến: ${routes
          .map((route) => `${route.from_location_id } - ${route.to_location_id}`)
          .join(", ")}. Bạn có thể cho mình biết tuyến bạn muốn đi không? Ví dụ: Hà Nội - Huế.`;
      }
      ...
      if (userInput.includes("hủy vé")) {
        ...
        if (ticketMatch) {
          ...
          return `Mình đã kiểm tra mã vé ${ticketId}. Vé của bạn đi từ ${ticket.route.start_location} đến ${ticket.route.end_location}, ngày ${ticket.departure_date}. Bạn có muốn hủy vé này không?`;
        }
